...
{
    [SerializeField]
    private LayerMask LayerMask; // checking layer
    [SerializeField]
    ...
    [SerializeField]
    [Range(0, 1f)]
    private float FadedAlpha = 0.6f; // Alpha value of fading
    [SerializeField]
    ...
    // run it on enable and not close
            ...
            foreach (Material material in FadingObject.Materials)
            {
                if (material.HasProperty("_Color"))
                {
                    material.color = new Color(
                        ...
                        material.color.g,
                        material.color.b,
                        Mathf.Lerp(FadedAlpha, FadingObject.InitialAlpha, time * FadeSpeed * 2)
                    );
                }
            }
