...
public class TreeReplacerS : EditorWindow {
	...
	public void Convert()
	{
		...
		for (int i = 0; i < data.treeInstances.Length; i++)
		{
			...
			// Vector3 position = new Vector3(tree.position.x * data.detailWidth - (data.size.x / 2), tree.position.y * y - 
            // (data.size.y / 2), tree.position.z * data.detailHeight - (data.size.z / 2));
